...
`IndexedChunk` is a flat, storage-agnostic retrieval projection of
`ContextualChunk`. It contains canonical chunk text, contextual embedding text,
a single section path, source attribution, lightweight citation fields,
flattened hierarchy, and tenant/tombstone governance. Detailed citation spans
and normalized visual geometry live in PostgreSQL and are resolved by stable
Item and chunk IDs only when a user opens a citation.
