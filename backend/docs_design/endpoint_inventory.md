...
Contract-first inventory. `Current` reflects registered routes before code
migration. `Final` follows `api_contract.md`.
...
| GET/POST | `/knowledge/collections` | mixed home + collection create | knowledge UI | `KnowledgeViewService`, `AdminConsoleService` | workspace/ACL | split | GET `/knowledge/home`; GET/POST `/collections` |
...
| POST | `/collections/{collection_id}/documents/upload` | direct multipart upload | knowledge/admin UI | `WorkspaceDocumentService.upload_to_collection` | collection write ACL | rename; merge with reservation | `/collections/{collection_id}/documents` (`multipart/form-data`) |
...
| GET/POST/PATCH/DELETE | `/admin/collections*` | Collection admin CRUD/ACL | admin UI | `AdminConsoleService`, `ItemCatalogService` | workspace admin | merge | `/collections*` |
| GET/POST/PATCH/DELETE | `/admin/items*` | mixed Item inventory/retry | admin UI | `ItemCatalogService` | item manage | split/merge | `/collections`, `/documents`, `/ingestions` |
| GET | `/admin/overview` | workspace dashboard | admin UI | `DashboardService.overview` | workspace read | move | `/workspaces/{workspace_id}/overview` |
| GET/PATCH | `/admin/spaces*` | workspace profile | admin UI | `TenantService` | workspace read/manage | rename | `/workspaces*` |
| GET/POST/PATCH | `/admin/users*` | workspace users | admin UI | `UserService` | user manage | move | `/users*` |
| GET/POST/PATCH/DELETE | `/admin/roles*` | workspace roles | admin UI | `RoleService` | role manage | move; DELETE becomes PATCH status | `/roles*` |
| GET/POST/PATCH/DELETE | `/admin/groups*` | workspace groups | admin UI | `GroupService` | group manage | move | `/groups*` |
| GET | `/admin/permissions` | permission catalog | admin UI | `RoleService.list_permissions` | role manage | move | `/permissions` |
| GET/POST/PATCH | `/admin/approval-requests*` | governance requests | admin UI + users | `ApprovalRequestService` | requester/reviewer | move | `/approval-requests*` |
| GET | `/admin/audit-logs` | workspace audit | admin UI | `AuditService.list_events` | audit read | move | `/audit-logs` |
| GET | `/admin/platform/overview` | platform dashboard | platform UI | `DashboardService.platform_overview` | platform permission | move | `/platform/overview` |
| GET | `/admin/platform/workspaces` | all workspaces | platform UI | `DashboardService.list_platform_workspaces` | platform permission | move | `/platform/workspaces` |
| GET | `/admin/platform/users` | all users | platform UI | `UserService.list_platform_users` | platform permission | move | `/platform/users` |
| GET | `/admin/platform/audit` | all audit | platform UI | `AuditService.list_platform_events` | platform permission | rename/move | `/platform/audit-logs` |
| GET | `/admin/platform/health` | platform health | platform UI | `HealthService` | platform permission | move | `/platform/health` |
| GET/POST/PATCH/DELETE | `/connections*` | connection resource | integrations UI | `IntegrationConsoleService` | connection capabilities | keep, rename IDs | `/connections*` |
| GET/POST/PATCH/DELETE | `/sources*` | source/schedule resource | integrations UI | `IntegrationConsoleService` | source capabilities | keep, remove action routes | `/sources*` |
| GET/POST | `/ingestions*` | ingestion runs | integrations UI | `IntegrationConsoleService`, Temporal adapter | source manage | rename IDs | `/ingestions*` |
| POST | `/sources/{source_id}/ingestions` | start source ingestion | integrations UI | `IntegrationConsoleService.ingest_source` | source manage | keep | same |
