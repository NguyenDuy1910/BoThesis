...
export function Pagination({
  ...
  return (
    ...
    >
      <p className="text-[0.75rem] tabular-nums text-[var(--text-tertiary)]">
      ...
      <div className="flex items-center gap-0.5">
        ...
        {pageWindow(page, totalPages).map((entry, index) =>
          entry === "gap" ? (
            <span
              aria-hidden="true"
              className="px-1 text-[0.75rem] text-[var(--text-tertiary)]"
              key={`gap-${index}`}
            >
              …
            </span>
            ...
            <button
              ...
              className={cn(
                "inline-flex h-7 min-w-7 items-center justify-center rounded-[var(--radius-xs)] px-1.5 text-[0.75rem] font-medium tabular-nums transition-colors",
                ui.focus,
                page === entry
