...
const isVisitor = (item: AuthWorkspace) => item.role_codes.includes("guest");
...
export function WorkspaceSwitcherDialog({ error, onClose, onSelect, open, switchingId, workspaces }: {
  ...
  open: boolean; switchingId: string | null; workspaces: AuthWorkspace[];
}) {
  const session = useAuthSession();
  const [search, setSearch] = useState("");
  const matches = workspaces.filter((workspace) => workspace.name.toLowerCase().includes(search.toLowerCase()));
  const groups = [
    { label: "Current", rows: matches.filter((item) => item.id === session?.active_workspace_id) },
    { label: "Your organizations", rows: matches.filter((item) => item.id !== session?.active_workspace_id && !isVisitor(item)) },
    { label: "Available to you", rows: matches.filter((item) => item.id !== session?.active_workspace_id && isVisitor(item)) },
  ];
  return (
    <Dialog className="max-w-sm" onClose={onClose} open={open} title="Switch workspace">
      ...
      <div className="max-h-[55vh] overflow-y-auto py-3">
        {groups.map((group) => group.rows.length > 0 && <section key={group.label} aria-label={group.label}>
          ...
          {group.rows.map((item) => <button key={item.id} className="flex min-h-14 w-full items-center gap-3 rounded-[var(--radius-sm)] p-2 text-left hover:bg-[var(--surface-hover)]" disabled={Boolean(switchingId)} onClick={() => item.id === session?.active_workspace_id ? onClose() : onSelect(item.id)} type="button">
