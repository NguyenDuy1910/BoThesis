...
export function useWorkspaceBreadcrumb() {
  const value = useContext(BreadcrumbContext);
  if (!value) {
    throw new Error("useWorkspaceBreadcrumb must be used inside WorkspaceBreadcrumbProvider");
  }
  return value;
}
...
export function useBreadcrumbDetail(title: string | undefined) {
  const { setDetailTitle } = useWorkspaceBreadcrumb();
  useEffect(() => {
