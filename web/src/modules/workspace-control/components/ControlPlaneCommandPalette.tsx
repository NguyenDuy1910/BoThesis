...
import {
  controlPlaneRoutes,
  canAccessControlPlaneRoute,
  type ControlPlaneRoute,
} from "@/modules/workspace-control/navigation";
...
function matches(route: ControlPlaneRoute, query: string) {
 ...
 */
export function ControlPlaneCommandPalette({
  open,
  onClose,
}: {
  open: boolean;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      controlPlaneRequest<{ items: Array<{ id: string; name: string }> }>(
        `/documents${queryString({ search: term, page_size: 6 })}`,
  ...
  const entries = useMemo<PaletteEntry[]>(() => {
    ...
    const sections = controlPlaneRoutes
      .filter((route) => canAccessControlPlaneRoute(route, getAuthSession()))
  ...
  return createPortal(
    <div className="ctl-palette" role="dialog" aria-label={`Search ${appBrand.productName} Control Plane`} aria-modal="true">
      <button
        aria-label="Close search"
        className="ctl-palette__scrim"
        onClick={onClose}
        tabIndex={-1}
        type="button"
      />
      <div className="ctl-palette__panel">
        <div className="ctl-palette__field">
        ...
        <div className="ctl-palette__results">
          {entries.length === 0 ? (
            <p className="ctl-palette__empty">
            ...
            entries.map((entry, index) => {
              ...
              return (
                <div key={entry.id}>
                  {showGroup && <p className="ctl-palette__group">{entry.group}</p>}
                  <button
                    className="ctl-palette__item"
